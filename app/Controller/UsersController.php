...
class UsersController extends AppController {
	public $name = "Users";
	var $uses = array ('User', 'TestHistory', 'Test', 'Lesson', 'InitialUser', 'Verifycode', 'InitialVerifycode', 'LockedUser', 'ChangeableValue' );
	...
	public function login() {
		...
		if ($this->request->is ( 'post' )) {
			...
			// debug($guest);die();
			if ($guest) {
				if ($guest ['LockedUser'] ['count'] >= ($this->ChangeableValue->field ( "current_value", array ('id' => 4 ) ) - 1)) {
					...
					if ($guest ['LockedUser'] ['lock_flg'] == 1) {
						if ((strtotime ( date ( "Y/m/d H:i:s" ) ) - strtotime ( $guest ['LockedUser'] ['lock_start_time'] )) < 60 * $this->ChangeableValue->field ( 'current_value', array ('id' => 5 ) )) {
						...
						$this->LockedUser->set ( array ('lock_flg' => 1, 'lock_start_time' => date ( "Y/m/d H:i:s" ) ) );
		...
		// 					}
			...
			if (! empty ( $user )) {
				...
				if ($user ['User'] ['level'] == 2 && $user ['User'] ['ip_address'] != NULL && $user ['User'] ['ip_address'] != $this->request->clientIp ()) {
					...
					$this->redirect ( array ('controller' => 'Users', 'action' => 'teacherVerify', $user ['User'] ['id'] ) );
				}
	...
	public function teacherName() {
		...
		if ($this->request->is ( "post" )) {
			if ($guest = $this->User->find ( 'first', array ('conditions' => array ('user_name' => $this->request->data ['User'] ['username'], 'level' => 2 ) ) )) {
				$this->redirect ( array ('controller' => 'Users', 'action' => 'teacherVerify', $guest ['User'] ['id'] ) );
			} else
	...
	public function teacherPass($id) {
		...
		$teacher = $this->User->find ( 'first', array ('conditions' => array ('id' => $id ) ) );
	...
	public function teacherVerify($id) {
		...
		if ($this->request->is ( 'post' )) {
			...
			if ($verifycode == $teacher ['Verifycode'] ['verifycode']) {
				$this->redirect ( array ('controller' => 'Users', 'action' => 'teacherPass', $id ) );
