	...
	function changeVerify() {
		...
		if ($this->request->is ( 'post' )) {
			...
			if (sha1 ( $this->Auth->user ( 'user_name' ) . $data ['User'] ['verifycode1'] . 'sha1' ) == $teacher ['Verifycode'] ['verifycode']) {
				$this->Verifycode->id = $this->Verifycode->find('first',array('conditions'=>array('user_id'=>$this->Auth->user ( 'id' ))))['Verifycode']['id'];
	...
	public function createNewCategory() {
		...
		if (isset ( $_POST ['name'] )) {
			$category = $this->Category->find ( 'first', array (
					'conditions' => array (
							strtolower ( 'category_name' ) => strtolower ( $_POST ['name'] ) 
					) 
			) );
			if ($category) {
